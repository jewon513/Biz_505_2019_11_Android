...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    /*
        menu 리소스를 현재 activity의 Appbar에 표시하기 위해서 메뉴를 설정하는 method
     */
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        // option menu 중에서 어떤 아이템이 선택되었는지 id값을 추출하는 부분
        int m_id = item.getItemId();

        if(m_id == R.id.m_login){

            // Intent는 현재 Activity에서 다른 Activity로 전환하기 위한 class
            // 생성자에 현재의 Activity와 새로 열릴 Activity.class를 매개변수로 전달한다.
    ...
    /*
        화면에 표시된 view에 contextMenu를 설정하기 위한 method이다.
        설정된 view에 따라 반응형 contextMenu 적용이 가능하다.

        ContextMenu 설정 순서
            1. menu.xml 리소스를 작성
            2. Activity에서 onCreateContextMenu() method를 정의한 후
               menu.xml 리소스를 Inflater 하여 설정
            3. onCreate() method 에서
               contextMenu를 적용할 view에 resiterForContext() 설정을 하면 된다.
     */
    @Override
