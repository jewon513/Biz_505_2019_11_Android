    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                /*
                    Intent : 안드로이드에서 보여지는 화면 (page) 다른 화면을 보이고자 할때는 Intent를 만들고 startActivity() method를 사용하여 intent를 열어준다.
                 */
...
//                startActivity(intent);
                ...
                //requestCode : 열리는 Activity와 열린 Activity가 서로 데이터를 교환할때 사용할 ID 값
                //startActivityForResult(intent, RequestCode.REQUEST_INPUT_CODE);


                // 폰에 저장된 이미지를 불러와서 content.xml의 ImageView에 표시를 하려고 한다.
                ...
                // 이미지를 저장하는 공간
                // 내장메모리, 외장 SD 카드가 있는데 모두 나에게 접근 할 수 있게 해달라는 설정
                imgIntent.setData(MediaStore.Images.Media.EXTERNAL_CONTENT_URI);

                // 이미지 보기가 열렸을때 이미지를 클릭하면 해당 이미지의 정보를 return 받기 위해서 Intent 열기
    ...
    // 현재 Activity레서 새로운 Activity를 열었을때 새로운 Activity가 어떤 값을 되돌려주면 그 값을 수신할 method
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        if(requestCode == RequestCode.REQUEST_IMAGE_CODE){
            // 이미지 갤러리가 보내는 데이터를 받아서 ImageView 에 이미지 표시
            ...
            if(resultCode == RequestCode.RESULT_OK){
                ...
                Toast.makeText(MainActivity.this,"입력오류 발생", Toast.LENGTH_LONG).show();
            }
